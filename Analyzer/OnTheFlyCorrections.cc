...
OnTheFlyCorrections::OnTheFlyCorrections(std::string gt, bool isdata){
  ...
  //std::string path="/cms/data/store/user/t2/users/takeimai/CMSSW_8_0_13/src/SUSYAnalyzer/PatAnalyzer/jetfiles/";
...
OnTheFlyCorrections::getCorrections(float rawpt, float raweta, float rawnomupt, float phi, float emf, float rho, float area, string level) {
  ...
  //  if(level=="")cout <<"EMfraction: " <<emf<<endl;
  if (emf > 0.9)       return corr;       // skip jets with EMF > 0.9

  //  if(level=="")cout<< "Pt, Eta, Rho, Area used for corrections: "<< rawpt<<", "<<raweta<<", " <<rho <<", "<<area<< endl ;
  ...
  //if(level=="")cout <<"L1corrpt, Fullcorrpt " << l1corrpt<<", "<<fullcorrpt<<endl;

  //  if (fullcorrpt < 10.)     return corr;        // skip all jets that have corrected pt below 10 GeV
  if (fullcorrpt < 15.)     return corr;        // skip all jets that have corrected pt below 15 GeV best shot
  //  if (rawpt < 15.)     return corr; 
  // if (l1corrpt < 15.)     return corr;  //not too bad
  // the corretions for the MET are the difference between l1fastjet and the full corrections on the jet!
  if(level=="up")   fullcorrpt  *= (1 + getJECUncertainty(rawpt,raweta));
  if(level=="down")   fullcorrpt  *= (1 - getJECUncertainty(rawpt,raweta));
