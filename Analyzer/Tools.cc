...
using namespace edm;
int misshitcuts = 2;// keep eles with miss hits <= misshitcuts
double tools::getPFIsolation(edm::Handle<pat::PackedCandidateCollection> pfcands,
    ...
    double Aeff[2][7] = {{ 0.1703, 0.1715, 0.1213, 0.1230, 0.1635, 0.1937, 0.2393 },{ 0.0735, 0.0619, 0.0465, 0.0433, 0.0577,0,0 }};
...
bool tools::triggerEmulator(const pat::Electron *iE) {
    ...
    //sIEtaIEta<0.031, H/E<0.08, |dEtaIn|<0.01, |dPhiIn|<0.08, |1/E - 1/P|<0.01
    return passed;
}
...
bool tools::triggerEmulatorReturned_EWKFOID(const pat::Electron *iE, double jetPt) {
    
    bool passed = true;
    double correctedpt=0.85*jetPt;
    double etasc = iE->superCluster()->eta();
    
    if (correctedpt>30&&(iE->hadronicOverEm()>=(0.10-0.03*(fabs(etasc)>1.479)))) passed = false;
    //if (iE->hcalOverEcal()>=(0.10-0.03*(fabs(etasc)>1.479))) passed = false;
    if (fabs(iE->deltaEtaSuperClusterTrackAtVtx())>=(0.01-0.002*(fabs(etasc)>1.479))) passed = false;
    if (fabs(iE->deltaPhiSuperClusterTrackAtVtx())>=(0.04+0.03*(fabs(etasc)>1.479))) passed = false;
    double eInvMinusPInv = iE->ecalEnergy()>0. ? (1.0/iE->ecalEnergy() - iE->eSuperClusterOverP()/iE->ecalEnergy()) : 9e9;
...
bool tools::isoTriggerEmulator(const pat::Electron *iE, double mvavalue) {
  
    double mvacut[3]={-0.155,-0.56,-0.76};
    /*
    double mvacut[3]={9999,9999,9999};
    if ((iE->pt())>=20) {
        mvacut[0]=-0.85;
        mvacut[1]=-0.88;
        mvacut[2]=-0.84;
  ...
  //Extra condition for FO:
  if (TMath::Abs(iE->superCluster()->eta()) < 0.8 && mvavalue < mvacut[0]) return false;
  else if (TMath::Abs(iE->superCluster()->eta()) < 1.479 && mvavalue < mvacut[1]) return false;
  else if(mvavalue<mvacut[2]) return false;
  
  if (iE->dr03TkSumPt()/iE->pt() > 0.2) return false;
  if (iE->hcalPFClusterIso()/iE->pt() > 0.25) return false; 
  if (iE->ecalPFClusterIso()/iE->pt() > 0.45) return false;
    
  return true;
}

bool tools::triggerEmulator(const pat::Muon *iM) {
  return true;
}
...
std::vector<const pat::Electron* > tools::ssbMVAElectronSelector(const std::vector<pat::Electron>  & thePatElectrons,
                                                              ...
                                                              bool bool_electron_chargeConsistency, bool useconversions,
                                                                 ...
                                                                 bool tight)
{
    ...
    */
	...
	//        if( vtxFitConversion )  continue;
        ...
        if( gsfTrack->hitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS) > misshitcuts ) continue;
					 ...
					 bool tight)
{
    ...
    bool vtxFitConversion = ConversionTools::hasMatchedConversion(reco::GsfElectron (*el), theConversions, BS);
    if( vtxFitConversion && useconversions)  return passed;
    
    if( gsfTrack->hitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS) >  misshitcuts ) return passed;
                                                              ...
                                                              reco::BeamSpot::Point BS)
{
    ...
    for( std::vector<pat::Electron>::const_iterator el = thePatElectrons.begin() ; el != thePatElectrons.end() ; el++ ) {
        ...
        */
        ...
        if( vtxFitConversion && useconversions) continue;
  ...
  for( std::vector<const pat::Electron *>::const_iterator el = thePatElectrons.begin() ; el != thePatElectrons.end() ; el++ )
    {
      ...
      float ecalIso = TMath::Abs((*el)->superCluster()->eta()) > 1.47 ? (*el)->dr03EcalRecHitSumEt() : TMath::Max((*el)->dr03EcalRecHitSumEt()-1.,0.);
...
std::vector<const pat::Tau* > tools::ssbLooseTauSelector(const std::vector<pat::Tau>  & thePatTaus,
                                                            double v_tau_pt,
                                                            reco::Vertex::Point PV,
                                                            double v_tau_d0,
                                                           const bool usedz)
{
    double v_tau_eta = 2.3;
       
    std::vector<const pat::Tau* > vTaus;
    for( std::vector<pat::Tau>::const_iterator tau = thePatTaus.begin() ; tau != thePatTaus.end() ; tau++ )
    {
   
        if ( tau->pt()  < v_tau_pt ) continue;
        if ( TMath::Abs( tau->eta() ) > v_tau_eta ) continue;
...
//Electrons
//ELE tightID/ISO for RA5/RA7
bool tools::TightID(TString theid, const pat::Electron *iE, edm::Handle< std::vector<reco::Conversion> > &theConversions, reco::BeamSpot::Point BS, reco::Vertex::Point PV, double mvavalue,double miniiso , bool use3DIPsig) {
    
    //Must pass Fake object selection
    if(!FOID(theid, iE, theConversions,BS,PV, mvavalue,miniiso, use3DIPsig)) return false;
    
    //Stronger MVA cuts:
    double mvacut[3]= {0.87,0.60,0.17 };
    /*
    double mvacut[3]={9999,9999,9999};
    if ((iE->pt())>=20) {
        mvacut[0]=0.51;
...
bool tools::TightIso(TString theid, const pat::Electron *iE, double mvavalue, double miniiso, double  ptratio, double ptrel ) {
  
  //Isolation 
  if(miniiso > 0.4) return false; 
  //RA5: tight WP for multiiso on electrons
  //See S13 here: https://www.dropbox.com/s/fsfw0gummwsc61v/lepawareJECv2_bkg_wp_300915.pdf?dl=0
  if(theid.Index("RA5")>=0&& miniiso > 0.12)  return false; 
  if(theid.Index("RA5")>=0&&ptratio < 0.80 && ptrel < 7.2  )return false; 
  //RA7: medium WP for multiiso on electrons 
  if(theid.Index("RA7")>=0&& miniiso > 0.16)  return false;
  if(theid.Index("RA7")>=0&&ptratio < 0.76 && ptrel < 7.2  )return false;
  return true;
}
      
//ELE TightID for EWK
bool tools::TightID_EWK(TString theid, const pat::Electron *iE, edm::Handle< std::vector<reco::Conversion> > &theConversions, reco::BeamSpot::Point BS, reco::Vertex::Point PV, double mvavalue, double lepmva, double miniiso, bool use3DIPsig) {
    
    //Pt cut @ 10
    //if ((iE->pt())<10) return false;

    double mvacut[3]= {-0.70,-0.83,-0.92};
    /*
    double mvacut[3]={9999,9999,9999};
    if ((iE->pt())>=20) {
        mvacut[0]=-0.96;
    ...
    //Eta:
    if(TMath::Abs(iE->superCluster()->eta()) >2.5) return false;
    //dz, dxy and 3DIP significance
    if( TMath::Abs(iE->gsfTrack()->dxy(PV)) > 0.05) return false;
    if( TMath::Abs(iE->gsfTrack()->dz(PV)) > 0.1) return false;
    double IP3Dsig = TMath::Abs(iE->dB(pat::Electron::PV3D)/iE->edB(pat::Electron::PV3D));
    if(theid.Index("EWK")>=0&&use3DIPsig&& IP3Dsig>8.) return false;
    
    //Conversion and missing hits
    bool vtxFitConversion = ConversionTools::hasMatchedConversion(reco::GsfElectron (*iE), theConversions, BS);
    int nbMissingInnerHits = iE->gsfTrack()->hitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS);
    if( vtxFitConversion || nbMissingInnerHits > 0 ) return false;

    
    //Isolation
    if(miniiso > 0.4) return false;
    
    if(theid.Index("EWK")>=0&&!triggerEmulatorReturned(iE)) return false;
    
    //Lepton MVA
    if (lepmva<=0.5) return false;

    return true;
}
      
//ELE FOID for RA5/RA7
bool tools::FOID(TString theid, const pat::Electron *iE, edm::Handle< std::vector<reco::Conversion> > &theConversions, reco::BeamSpot::Point BS, reco::Vertex::Point PV, double mvavalue, double miniiso, bool use3DIPsig) {
    ...
    if( TMath::Abs(iE->gsfTrack()->dz(PV)) > 0.1) return false;
    double IP3Dsig = TMath::Abs(iE->dB(pat::Electron::PV3D)/iE->edB(pat::Electron::PV3D));
    if(theid.Index("RA5")>=0&&use3DIPsig&& IP3Dsig>4.) return false;
    if(theid.Index("RA7")>=0&&use3DIPsig&& IP3Dsig>4.) return false;
    ...
    //Charge consistency
    //Only for RA5
    if(theid.Index("RA5")>=0&&!iE->isGsfCtfScPixChargeConsistent()) return false;
    
    //Isolation
    if(miniiso > 0.4) return false;
    
    if(theid.Index("RA5")>=0&&!triggerEmulatorReturned(iE)) return false;
    if(theid.Index("RA7")>=0&&!triggerEmulatorReturned(iE)) return false;
...
//ELE FOID for EWK
bool tools::FOID_EWK(TString theid, const pat::Electron *iE, edm::Handle< std::vector<reco::Conversion> > &theConversions, reco::BeamSpot::Point BS, reco::Vertex::Point PV, double mvavalue, double lepmva, double ptratio, double jetCSV, double jetPt, double miniiso, bool use3DIPsig) {
    ...
    int nbMissingInnerHits = iE->gsfTrack()->hitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS);
    //if( vtxFitConversion || nbMissingInnerHits > 0 ) return false;
    if( nbMissingInnerHits > 0 ) return false;
    
    //Isolation
    if(miniiso > 0.4) return false;
    
    if(theid.Index("EWK")>=0&&!triggerEmulatorReturned_EWKFOID(iE,jetPt)) return false;
    
    if (lepmva>0.5) {//MVA requirement met
        double mvacut[3]= {-0.70,-0.83,-0.92};
        /*
        double mvacut[3]={9999,9999,9999};
        if ((iE->pt())>=20) {
    ...
    else{//MVA requirement not met
        double mvacut[3]= {0,0,0.3};
        //MVA Tight WP
        if (TMath::Abs(iE->superCluster()->eta()) < 0.8 && mvavalue < mvacut[0]) return false;
        else if (TMath::Abs(iE->superCluster()->eta()) < 1.479 && mvavalue < mvacut[1]) return false;
        else if(mvavalue<mvacut[2]) return false;
        //Ptratio/JetCSV
        if (ptratio<0.3) return false;
        if (jetCSV>0.3) return false;
    }
...
//ELE VetoID for RA5/RA7
bool tools::VetoID(const pat::Electron *iE, edm::Handle< std::vector<reco::Conversion> > &theConversions, reco::BeamSpot::Point BS, reco::Vertex::Point PV, double mvavalue, double miniiso) {
    ...
    //dz, dxy (3DIP significance not for veto)
    if( TMath::Abs(iE->gsfTrack()->dxy(PV)) > 0.05) return false;
    if( TMath::Abs(iE->gsfTrack()->dz(PV)) > 0.1) return false;
    //double IP3Dsig = TMath::Abs(iE->dB(pat::Electron::PV3D)/iE->edB(pat::Electron::PV3D));
    //if(IP3Dsig>4) return false;
    
    //Conversion and missing hits
    bool vtxFitConversion = ConversionTools::hasMatchedConversion(reco::GsfElectron (*iE), theConversions, BS);
    int nbMissingInnerHits = iE->gsfTrack()->hitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS);
    if( vtxFitConversion || nbMissingInnerHits > 1 ) return false;
    
    //Charge consistency: not for veto
    //Tight charge for Ele
    //if(!iE->isGsfCtfScPixChargeConsistent()) return false;
    
    //Isolation
    if(miniiso > 0.4) return false;
    
    //TriggerEmulator
    if(!triggerEmulatorReturned(iE)) return false;
...
//ELE VetoID for EWK
bool tools::VetoID_EWK(const pat::Electron *iE, edm::Handle< std::vector<reco::Conversion> > &theConversions, reco::BeamSpot::Point BS, reco::Vertex::Point PV, double mvavalue, double miniiso) {
    ...
    if( TMath::Abs(iE->gsfTrack()->dz(PV)) > 0.1) return false;
    double IP3Dsig = TMath::Abs(iE->dB(pat::Electron::PV3D)/iE->edB(pat::Electron::PV3D));
    if(IP3Dsig>8) return false;
    
    //Conversion and missing hits
    //bool vtxFitConversion = ConversionTools::hasMatchedConversion(reco::GsfElectron (*iE), theConversions, BS);
    int nbMissingInnerHits = iE->gsfTrack()->hitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS);
    //if( vtxFitConversion || nbMissingInnerHits > 1 ) return false;
    if( nbMissingInnerHits > 1 ) return false;
...
bool tools::EleCutBasedLooseID_MiniIso(const pat::Electron *iE, edm::Handle< std::vector<reco::Conversion> > &theConversions, reco::BeamSpot::Point BS, reco::Vertex::Point PV, double miniiso) {
  
  //cf https://twiki.cern.ch/twiki/bin/view/CMS/CutBasedElectronIdentificationRun2#Spring15_selection_25ns
  //Eta: 
  if(TMath::Abs(iE->superCluster()->eta()) >2.5) return false;  
  else if( TMath::Abs(iE->superCluster()->eta()) < 1.479  ) {
    if( TMath::Abs(iE->full5x5_sigmaIetaIeta()) > 0.0103 ) return  false;
    if( TMath::Abs(iE->deltaEtaSuperClusterTrackAtVtx()) > 0.0105 ) return  false;
    if( TMath::Abs(iE->deltaPhiSuperClusterTrackAtVtx()) > 0.115  ) return  false;
    if( TMath::Abs(iE->hadronicOverEm())  > 0.104  ) return  false;
    if( TMath::Abs(1.0/iE->ecalEnergy() - iE->eSuperClusterOverP()/iE->ecalEnergy() )>0.102 ) return  false; 
    if( TMath::Abs(iE->gsfTrack()->dxy(PV)) > 0.0261) return false;
    if( TMath::Abs(iE->gsfTrack()->dz(PV)) > 0.41) return false;
    if(iE->gsfTrack()->hitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS)>2) return  false;
  }
  else {
    if( TMath::Abs(iE->full5x5_sigmaIetaIeta()) > 0.0301 ) return  false;
    if( TMath::Abs(iE->deltaEtaSuperClusterTrackAtVtx()) > 0.00814 ) return  false;
    if( TMath::Abs(iE->deltaPhiSuperClusterTrackAtVtx()) > 0.182  ) return  false;
    if( TMath::Abs(iE->hadronicOverEm())  > 0.0897  ) return  false;
    if( TMath::Abs(1.0/iE->ecalEnergy() - iE->eSuperClusterOverP()/iE->ecalEnergy() )>0.126 ) return  false; 
    if( TMath::Abs(iE->gsfTrack()->dxy(PV)) > 0.0118) return false;
    if( TMath::Abs(iE->gsfTrack()->dz(PV)) > 0.822) return false;
...
bool tools::EleCutBasedTightID(const pat::Electron *iE, edm::Handle< std::vector<reco::Conversion> > &theConversions, reco::BeamSpot::Point BS, reco::Vertex::Point PV) {
  
  //cf https://twiki.cern.ch/twiki/bin/view/CMS/CutBasedElectronIdentificationRun2#Spring15_selection_25ns
  //Eta: 
  if(TMath::Abs(iE->superCluster()->eta()) >2.5) return false;  
  else if( TMath::Abs(iE->superCluster()->eta()) < 1.479  ) {
    if( TMath::Abs(iE->full5x5_sigmaIetaIeta()) > 0.0101 ) return  false;
    if( TMath::Abs(iE->deltaEtaSuperClusterTrackAtVtx()) > 0.00926 ) return  false;
    if( TMath::Abs(iE->deltaPhiSuperClusterTrackAtVtx()) > 0.0336  ) return  false;
    if( TMath::Abs(iE->hadronicOverEm())  > 0.0597 ) return  false;
    if( TMath::Abs(1.0/iE->ecalEnergy() - iE->eSuperClusterOverP()/iE->ecalEnergy() )>0.012 ) return  false; 
    if( TMath::Abs(iE->gsfTrack()->dxy(PV)) > 0.0111) return false;
    if( TMath::Abs(iE->gsfTrack()->dz(PV)) > 0.0466) return false;
    if(iE->gsfTrack()->hitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS)>2) return  false;
  }
  else {
    if( TMath::Abs(iE->full5x5_sigmaIetaIeta()) > 0.0279 ) return  false;
    if( TMath::Abs(iE->deltaEtaSuperClusterTrackAtVtx()) > 0.00724 ) return  false;
    if( TMath::Abs(iE->deltaPhiSuperClusterTrackAtVtx()) > 0.0918  ) return  false;
    if( TMath::Abs(iE->hadronicOverEm())  > 0.0615  ) return  false;
    if( TMath::Abs(1.0/iE->ecalEnergy() - iE->eSuperClusterOverP()/iE->ecalEnergy() )>0.00999 ) return  false; 
    if( TMath::Abs(iE->gsfTrack()->dxy(PV)) > 0.0351) return false;
    if( TMath::Abs(iE->gsfTrack()->dz(PV)) > 0.417) return false;
    if(iE->gsfTrack()->hitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS)>1) return  false;
...
//Mu tightID/ISO for RA5/RA7
bool tools::TightID(TString theid, const pat::Muon *iM, reco::Vertex::Point PV,  double miniiso, bool use3DIPsig ) {
  
  //Must pass Fake object selection
  if(!FOID(theid, iM,PV,miniiso,use3DIPsig)) return false;
  
     
  return true;
}
      
bool tools::TightIso(TString theid,const pat::Muon *iM,  double miniiso, double  ptratio, double ptrel ) {
  
  //Isolation 
  if(miniiso > 0.4) return false; 
  //RA5: medium WP for multiiso on muons
  //See S13 here: https://www.dropbox.com/s/fsfw0gummwsc61v/lepawareJECv2_bkg_wp_300915.pdf?dl=0                                                                                                             
  if(theid.Index("RA5")>=0&& miniiso > 0.16)  return false;
  if(theid.Index("RA5")>=0&&ptratio < 0.76 && ptrel < 7.2  )return false;
  //RA7: loose WP for multiiso on muons                                                                                                                                                                 
  if(theid.Index("RA7")>=0&& miniiso > 0.2)  return false;
  if(theid.Index("RA7")>=0&&ptratio < 0.69 && ptrel < 6.0  )return false;

  return true;
}

//Mu TightID for EWK
bool tools::TightID_EWK(TString theid, const pat::Muon *iM, reco::Vertex::Point PV, double lepmva, double miniiso, bool use3DIPsig) {
    
    //Pt cut @ 10
    //if ((iM->pt())<10) return false;
    
    //Eta:
    if(TMath::Abs(iM->eta()) >2.4) return false;
    
    //Loose Muon ID:
    if ( !(iM->isGlobalMuon() || iM->isTrackerMuon() )) return false;
    if ( !(iM->isPFMuon()) ) return false;
    if(iM->innerTrack().isNull()) return false;
    
    
    //Medium Muon ID:
    bool goodGlob = iM->isGlobalMuon() && iM->globalTrack()->normalizedChi2() < 3
    && iM->combinedQuality().chi2LocalPosition < 12 && iM->combinedQuality().trkKink < 20;
    bool good =  iM->innerTrack()->validFraction() > 0.8 &&
    muon::segmentCompatibility(*iM) > (goodGlob ? 0.303 : 0.451);
    if(!good) return false;
    
    //dz, dxy and 3DIP significance
    if( TMath::Abs(iM->innerTrack()->dxy(PV))  > 0.05) return false;
    if( TMath::Abs(iM->innerTrack()->dz(PV))   > 0.1) return false;
    double IP3Dsig = TMath::Abs(iM->dB(pat::Muon::PV3D)/iM->edB(pat::Muon::PV3D));
    if(use3DIPsig&&IP3Dsig>8) return false;
    
    //Isolation
    if(miniiso > 0.4) return false;
    
    if(theid.Index("EWK")>=0&&!triggerEmulator(iM)) return false;
    
    
    //Lepton MVA
    if (lepmva <= -0.2) return false;
...
//Mu FOID for RA5/RA7
bool tools::FOID(TString theid, const pat::Muon *iM, reco::Vertex::Point PV, double miniiso, bool use3DIPsig) {
  ...
  if( TMath::Abs(iM->innerTrack()->dz(PV))   > 0.1) return false;
  double IP3Dsig = TMath::Abs(iM->dB(pat::Muon::PV3D)/iM->edB(pat::Muon::PV3D));
  if(use3DIPsig&&IP3Dsig>4) return false;
  
  //Charge consistency
  //Tight-charge for Mu
  //Only in RA5
  if(theid.Index("RA5")>=0&& TMath::Abs( iM->innerTrack()->ptError()/ iM->innerTrack()->pt()) > 0.2 ) return false;
  
  //Isolation 
  if(miniiso > 0.4) return false; 

  if(theid.Index("RA5")>=0&&!triggerEmulator(iM)) return false;
  if(theid.Index("RA7")>=0&&!triggerEmulator(iM)) return false;
  return true;
}

//Mu FOID for EWK
bool tools::FOID_EWK(TString theid, const pat::Muon *iM, reco::Vertex::Point PV, double lepmva, double ptratio, double jetCSV, double miniiso, bool use3DIPsig) {
    ...
    if (lepmva <= -0.2) {//LepMVA requirement not met
        //Ptratio/JetCSV
        if (ptratio<0.3) return false;
        if (jetCSV>0.3) return false;
    }
...
//Mu VetoID for RA5/RA7
bool tools::VetoID(const pat::Muon *iM, reco::Vertex::Point PV, double miniiso) {
  //Eta: 
  if(TMath::Abs(iM->eta()) >2.4) return false;  
  ...
  //dz, dxy (3DIPsig not needed)
  if( TMath::Abs(iM->innerTrack()->dxy(PV))  > 0.05) return false;
  if( TMath::Abs(iM->innerTrack()->dz(PV))   > 0.1) return false;
  //  double IP3Dsig = TMath::Abs(iM->dB(pat::Muon::PV3D)/iM->edB(pat::Muon::PV3D));
  //  if(IP3Dsig>4) return false; Not for Veto ID
  
  //Charge consistency Not for Veto ID
  //Tight-charge for Mu
  //  if( TMath::Abs( iM->innerTrack()->ptError()/ iM->innerTrack()->pt()) > 0.2 ) return false;
...
//Mu VetoID for EWK
bool tools::VetoID_EWK(const pat::Muon *iM, reco::Vertex::Point PV, double miniiso) {
    //Eta:
    if(TMath::Abs(iM->eta()) >2.4) return false;
    
    //Loose Muon ID:
    //Equivalent to iM->isLooseMuon():
    if ( !(iM->isGlobalMuon() || iM->isTrackerMuon() )) return false;
    if ( !(iM->isPFMuon()) ) return false;
    if(iM->innerTrack().isNull()) return false;
...
bool tools::SoftTightID(const pat::Muon *iM, reco::Vertex::Point PV, reco::Vertex thevtx) {
  //Eta: 
  if(TMath::Abs(iM->eta()) >2.4) return false;  


  if( !muon::isGoodMuon(*iM, muon::TMOneStationTight))return false;
  if(iM->innerTrack()->hitPattern().trackerLayersWithMeasurement() <= 5)return false;
  if(iM->innerTrack()->hitPattern().pixelLayersWithMeasurement() <=0) return false;
  /*
  //Loose Muon ID:
  if ( !(iM->isGlobalMuon() || iM->isTrackerMuon() )) return false;
  if ( !(iM->isPFMuon()) ) return false;
  if(iM->innerTrack().isNull()) return false;
  if ( !(iM->isSoftMuon(thevtx)) ) return false;
  */
  //dz, dxy and 3DIP significance
  if( TMath::Abs(iM->innerTrack()->dxy(PV))  > 0.01) return false;
  if( TMath::Abs(iM->innerTrack()->dz(PV))   > 0.01) return false;
...
bool tools::FOID(TString theid, const pat::Tau *iT, reco::Vertex::Point PV, bool use3DIPsig) {
  

  //Eta: 
  if(TMath::Abs(iT->eta()) >2.4) return false;  
  
  if (! (iT->tauID("againstElectronLooseMVA6")>0.5
	 && iT->tauID("againstMuonLoose3")>0.5
	 && iT->tauID("byVLooseIsolationMVArun2v1DBoldDMwLT")>0.5
	 && iT->tauID("decayModeFinding")>0.5 ) ) return false;

  reco::Vertex::Point vtx = (*(iT->leadChargedHadrCand())).vertex();
  TLorentzVector p4; p4.SetPtEtaPhiE(iT->pt(),iT->eta(),iT->phi(),iT->energy());
  double dzTau = (vtx.z()-PV.z()) - ((vtx.x()-PV.x())*p4.X()+(vtx.y()-PV.y())*p4.Y())/ p4.Pt() *  p4.Z()/ p4.Pt();
  if(dzTau>0.2) return false; 
  return true;  
}
...
bool tools::TightID(TString theid, const pat::Tau *iT, reco::Vertex::Point PV, bool use3DIPsig) {
  if(!FOID(theid, iT,PV,use3DIPsig))return false;
  return true; 
  
}

bool tools::TightIso(  const pat::Tau *iT ) {
  if ( ! ( iT->tauID("byTightIsolationMVArun2v1DBoldDMwLT")>0.5 ) ) return false;
...
bool tools::MuonLooseID_MiniIso(const pat::Muon *iM, reco::Vertex::Point PV, double miniiso) {
  //Eta: 
  if(TMath::Abs(iM->eta()) >2.4) return false;  
  ...
  //dz, dxy and 3DIP significance
  if( TMath::Abs(iM->innerTrack()->dxy(PV))  > 0.2) return false;
  if( TMath::Abs(iM->innerTrack()->dz(PV))   > 0.5) return false;
...
bool tools::PassMETFilters(const edm::Event& iEvent, edm::EDGetTokenT<edm::TriggerResults> metfilters_token, edm::EDGetTokenT<bool>filterbadChCand_token,edm::EDGetTokenT<bool>filterbadPFMuon_token){
  edm::Handle<TriggerResults> METFilterResults;

  iEvent.getByToken(metfilters_token, METFilterResults);
  //  if( METFilterResults.failedToGet() ) cout << "--- NO METFilter RESULTS !! ---" << endl;
  if( !METFilterResults.failedToGet() ) {
	...
	//if(MetfilterPath.Index("Flag_eeBadScFilter")>=0) 
	//cout << MetfilterPath<< endl;
	if(
	   MetfilterPath.Index("Flag_globalTightHalo2016Filter")>=0 || 
	   MetfilterPath.Index("Flag_HBHENoiseFilter")>=0 ||
	   MetfilterPath.Index("Flag_HBHENoiseIsoFilter")>=0 ||
	   MetfilterPath.Index("Flag_EcalDeadCellTriggerPrimitiveFilter")>=0 ||
	   MetfilterPath.Index("Flag_goodVertices")>=0 ||
	   MetfilterPath.Index("Flag_eeBadScFilter")>=0 
	   //	   MetfilterPath.Index("")>=0 ||
	   //  MetfilterPath.Index("")>=0 ||
  ...
  iEvent.getByToken(filterbadChCand_token, ifilterbadChCand);
  bool  filterbadChCandidate = *ifilterbadChCand;
    
  Handle<bool> ifilterbadPFMuon;
  iEvent.getByToken(filterbadPFMuon_token, ifilterbadPFMuon);
  bool filterbadPFMuon = *ifilterbadPFMuon;

  if(!filterbadPFMuon||!filterbadChCandidate) return false;
